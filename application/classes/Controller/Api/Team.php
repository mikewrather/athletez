 ...
 */
	...
	class Controller_Api_Team extends Controller_Api_Base
	{
		...
		public function action_get_recent_schedules()
		{
			...
			{
				$display_qty = (int)trim($this->request->query('display_qty'));
			}else{
				$e = Kohana::$config->load('sysconfig');
				$display_qty = $e->get('defaultCommentsToDisplay');
			}
			$std = new stdClass();
			$std->result = $this->mainModel->getSchedule($display_qty, true);
		...
		public function action_get_upcoming_schedules()
		{
			...
			$std = new stdClass();
			$std->result = $this->mainModel->getSchedule($display_qty);
		...
		/**
		 * This has been moved to the parent class so that we can add comments to any type of entity.
		 *
		 * @return bool|void
		public function action_post_addcomments()
		{
			parent::action_post_addcomments();
		}
		*/
