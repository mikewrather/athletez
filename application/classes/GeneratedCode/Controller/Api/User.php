 ...
 */
	...
	class Controller_Api_User extends Controller_Api_Base
	{
		...
		public function action_post_addteam()
		{
			...
			// seasons_arr 
			// Array of seasons to add.  This will create multiple teams with one addteam call.
				
			if(isset($this->request->post('seasons_arr')))
			{
				$arguments["seasons_arr"] = $this->request->post('seasons_arr');
				foreach($arguments["seasons_arr"] as $seasons_arr_key =>$seasons_arr_val)
				{
					// Access each item in the array through the $seasons_arr_val variable
				}
			}
		...
		/**
		 * action_post_fbreg() Register a new user with Facebook oAuth.  This will pull name, email, gender, birthday, and potentially high school for registration purpose.
		 * via /api/user/fbreg/{users_id}
		 *
		 */
		public function action_post_fbreg()
		{
			$this->payloadDesc = "Register a new user with Facebook oAuth.  This will pull name, email, gender, birthday, and potentially high school for registration purpose.";
			$arguments = array();
