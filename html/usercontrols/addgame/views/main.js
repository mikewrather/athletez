	...
	AddGameView = SectionView.extend({
		...
		render : function() {
			...
			var form = new FormComponent({
		        date: {type: 'Text', attr: { 'placeholder': 'Enter Date', 'class': "txt-game-date_h txtDate"}, showLable: false, validators: ['required'], bindDatePicker: true, chnageEvent: function() {
		        ...
		        'Day_light': {
	        		...
	        		options: {
						data: { records: [{
							...
							}], recordId: 'name', recordValue: 'value', selectedValue : "PM"},
	        		...
	        		options: {
						...
						callback: function(result) {
							...
							if (result && result != 0) {
								...
								if(_self.getTeamType(result)) _self.fillTeams(result);
						} else {
				...
				'Select_Team_1': {
	        		...
	        		options: {
						...
						elementId: _self.controls.hdnTimePeriodData,
						getData: function(value, callback) {
							_self.teamsCollectionList = new UserTeamsCollection();
							_self.teamsCollectionList.user_id = _self.user_id;
							_self.teamsCollectionList.sports_id = $(".selected-sport-h").data("id");
							_self.teamsCollectionList.processResult = function(collection) {
								var item = {
									team_name : "Team Not Found",
									id : -1
								};
								var list = _self.teamsCollectionList.AppendItem(item);
								if(callback) callback(list);
							};
							_self.teamsCollectionList.fetch();
						},
				...
				'score_1': {type: 'Text', attr: { 'placeholder': 'Score', 'class': "txt-score-team-h hidden"}, showLable: false},
				...
				'score_2': {type: 'Text', attr: { 'placeholder': 'Score', 'class': "txt-score-team-h hidden"}, showLable: false},
